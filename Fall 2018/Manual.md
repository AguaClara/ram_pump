...
## Introduction
The ram pump is a gravity power pump that will provide a convenient automatic movement of water around AguaClara plants from low to high elevations in order to provide clean, treated water to the plumbing system of the plant which provides water for chemical dosage systems as well as sinks and toilets. This tool will alleviate the burden of manually having to carry water from the plant effluent up to other parts of the plant. The problem with the current prototype is its inefficiency which can be attributed to a variety of factors ranging from head loss to an inefficient spring or inefficient calculated spring constant. Previously during Spring 2018, the AguaClara team focused on creating a mathematical model to find the optimal spring constant for the ram pump. During the Fall 2018 semester, the Ram Pump subteam will build on these previously proposed models so that a more accurate value for the spring constant can be found. This analysis will allow for an efficient way to finding the optimal spring for any given system.
...
In order to find the maximum k value that the pump can function with, it is necessary to know the maximum force that would be exerted on the plate at the target maximum velocity in order to complete a force balance. In the past, the target velocity of $\frac{1}{2}V_f$ was arbitrarily chosen. A target velocity below the final velocity of the system may be necessary because the velocity within the drive pipe has an asymptote at the final velocity and it may take a relatively long time to reach that value or very close to it. In order to avoid having to wait to reach that value, the ram pump can be designed with a smaller target velocity for which the plate will close. The team has not yet looked into the ideal target velocity but continued with the choice of $\frac{1}{2}V_f$. Everything that the team did using $\frac{1}{2}V_f$ can be replicated for any other target velocity.
